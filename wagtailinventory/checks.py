...
def dal_select2_check_all():
    ...
    return (
        "dal" in settings.INSTALLED_APPS
        and "dal_select2" in settings.INSTALLED_APPS
    )
...
def dal_select2_system_check(app_configs, **kwargs):
    ...
    if not dal_select2_check_all():
        errors.append(
            Warning(
                (
                    "django-autocomplete-light is not installed or "
                    "configured. Block inventory will fall back on built-in "
                    "select widgets."
                ),
                hint=(
